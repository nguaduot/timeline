...
namespace Timeline.Providers {
    public class ToopicProvider : BaseProvider {
        // 页数据索引（从1开始）（用于按需加载）
        private int pageIndex = 1;

        private const string URL_API = "https://api.nguaduot.cn/toopic/v2?client=timelinewallpaper&cate={0}&order={1}&page={2}&unaudited={3}&marked={4}";
        
        private Meta ParseBean(ToopicApiData bean, string order) {
            Meta meta = new Meta {
                Id = bean.Id,
                Uhd = bean.ImgUrl,
                Thumb = bean.ThumbUrl,
        ...
        public override async Task<bool> LoadData(CancellationToken token, BaseIni bi, int index, DateTime date = new DateTime()) {
            ...
            try {
                ...
                foreach (ToopicApiData item in api.Data) {
