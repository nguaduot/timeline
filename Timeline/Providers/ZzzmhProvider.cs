...
namespace Timeline.Providers {
    public class ZzzmhProvider : BaseProvider {
        private const string URL_API = "https://api.nguaduot.cn/zzzmh/v2" +
            "?client=timelinewallpaper&device={0}" +
            "&order={1}&cate={2}" +
            "&tag={3}&no={4}&date={5}&score={6:F4}&admin={7}";

        private Meta ParseBean(ZzzmhApiData bean) {
            Meta meta = new Meta {
                Id = bean.Id,
                No = bean.No,
                Uhd = bean.ImgUrl,
        ...
        public override async Task<bool> LoadData(CancellationToken token, Ini ai, BaseIni bi, Go go) {
            ...
            try {
                ...
                foreach (ZzzmhApiData item in api.Data) {
