...
+ 新增：图源 `Simple Desktops`
+ 修复：图源 `Wallpaper Abyss` 推送不生效

v8.9.230204

+ 重构：重新设计图源 `贪食鬼`，设置 `周度精选` 开关，开即原 `周选集`，关即大合集，融合10+图源2w+图片，饕餮也
+ 新增：图源 `Wallpaper Abyss`
+ 调整：设置页图源分为两组
+ 调整：排序 `收录` 更名为 `最新`，即最近收录图片
+ 修复：翻页后双击图片切换全屏/窗口功能失效

v8.3.230128

+ 新增：图源 `Skitterphoto`，主打摄影
+ 新增：图源 `美图集`，每日更新几张精选图片（不可见）
+ 调整：优化图源 `WallpaperUP` 并恢复可见
+ 调整：排序 `喜欢` 更名为 `趋势`，即近期热门图片

v8.0.221209

+ 新增：图源 `极简壁纸`
+ 移除：从可见图源中移除 `WallpaperUP`（仍可在配置文件中启用）
+ 优化：配置文件打不开时打开所在文件夹
+ 优化：图源 `wallhaven` 显示标签、上传者
+ 调整：图源 `wallhaven` 新增分类 `People`

v7.9.221127

+ 修复：低于 `1` 小时的推送周期未生效
+ 优化：`本地图库` 分批加载，避免大量文件耗时过长
+ 优化：重新设计翻页动画，减弱亮色主题下翻页闪白造成的视觉刺激
+ 优化：壁纸缓存调整为在推送时清理（浏览缓存仍在启动时清理）
+ 优化：缓存推送壁纸时删除冗余
+ 新增：图源 `backiee`
+ 新增：统计参数新增 `pc`，指示设备为台式或笔记本电脑

v7.7.221106

+ 修复：`本地图库` 推送默认情况未跟随收藏夹
+ 修复：图源 `wallhaven` 推送服务统计数据上传错误
+ 新增：统计参数新增 `arch`，指示本应用所打包架构
